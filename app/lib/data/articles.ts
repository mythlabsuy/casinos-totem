...
export async function fetchFilteredArticles(
  ...
  try {
    const searchParams = new URLSearchParams({ skip: offset.toString(), limit: ITEMS_PER_PAGE.toString(), query: query })
    const response = await apiFetchServer({method: 'GET', path: 'article', body: undefined, query: searchParams});

    console.log("ARTICLES RESP", response);
...
export async function fetchArticlesPages(search_query: string) {
  try {
    ...
    const response = await apiFetchServer({method: 'GET', path: 'article/count', body: undefined, query: query});
    const articles_amount = await response.json();

    const totalPages = Math.ceil(articles_amount / ITEMS_PER_PAGE);
    return totalPages;
  } catch (error) {
    console.error('Database Error:', error);
    throw new Error('Failed to fetch total number of invoices.');
...
export async function fetchArticle(id: number) {
  try {
    const response = await apiFetchServer({method: 'GET', path: `article/${id}`, body: undefined});
    return response;
  } catch (error) {
    console.error('Database Error:', error);
    throw new Error('Failed to fetch total number of invoices.');
